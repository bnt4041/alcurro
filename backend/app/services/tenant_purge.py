...
def purge_clock_ins(session: Session, tenant_id: UUID) -> int:
    ...
    result = session.exec(
        delete(ClockIn).where(
            col(ClockIn.employee_id).in_(
                select(Employee.id).where(col(Employee.company_id).in_(company_ids))
            )
        )
    )
    session.flush()
...
def purge_work_breaks(session: Session, tenant_id: UUID) -> int:
    """Elimina todas las paradas de un tenant."""
    company_ids = _company_ids(session, tenant_id)
    if not company_ids:
        return 0
    result = session.exec(
        delete(WorkBreak).where(
            col(WorkBreak.employee_id).in_(
                select(Employee.id).where(col(Employee.company_id).in_(company_ids))
            )
        )
    )
...
def purge_leave_requests(session: Session, tenant_id: UUID) -> int:
    ...
    result = session.exec(
        delete(LeaveRequest).where(
            col(LeaveRequest.employee_id).in_(
                select(Employee.id).where(col(Employee.company_id).in_(company_ids))
            )
        )
    )
...
def purge_employees(session: Session, tenant_id: UUID) -> int:
    """Elimina todos los empleados y sus datos asociados de un tenant.

    Orden: primero se borran las tablas hijas, luego los empleados.
    """
    company_ids = _company_ids(session, tenant_id)
    if not company_ids:
        return 0
    ...
    # 1. Datos dependientes de empleados
    ...
    # DocumentDelivery: eliminar tags y luego los registros
    doc_ids = session.exec(
        select(DocumentDelivery.id).where(
            col(DocumentDelivery.employee_id).in_(emp_subquery)
        )
    ).all()
    if doc_ids:
        session.exec(
    ...
    # Firmas: eliminar OTPS, eventos, signers, envelopes
    signer_ids = session.exec(
        select(SignatureSigner.id).where(
            col(SignatureSigner.employee_id).in_(emp_subquery)
        )
    ).all()
    if signer_ids:
        session.exec(
