...
export default function PlatformPaddlePage() {
  const { notify } = useToast();
  const [status, setStatus] = useState<PaddleStatus | null>(null);
  const [payments, setPayments] = useState<PaddlePayment[]>([]);
  const [loading, setLoading] = useState(true);
  const [actionLoading, setActionLoading] = useState<string | null>(null);

  const load = useCallback(async () => {
    const [s, p] = await Promise.all([
      api.get<PaddleStatus>("/platform/paddle/status"),
      api.get<PaddlePayment[]>("/platform/paddle/payments"),
    ]);
    setStatus(s);
    setPayments(p);
  }, []);
  ...
  const handleRefund = async (row: PaymentRow) => {
    ...
    try {
      const res = await api.post<RefundResult>(`/platform/paddle/refund/${row.id}`, {});
  ...
  const columns = useMemo<DataTableColumn<PaymentRow>[]>(
    () => [
      ...
      {
        title: "Factura",
        field: "paddle_transaction_id",
        width: 100,
        formatter: (cell) => {
          const r = cell.getRow().getData() as PaymentRow;
          if (!r.receipt_url) return `<span class="muted small">—</span>`;
          const ref = r.paddle_transaction_id || r.paddle_invoice_id || "";
          return `<div style="display:flex;gap:6px;align-items:center">
            <a href="${r.receipt_url}" target="_blank" rel="noopener" title="Abrir factura en Paddle" style="color:var(--color-primary,#27ae60);display:flex;align-items:center">
        ...
        {loading ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", flexWrap: "wrap" }}>
              ...
              <span className={`badge ${status.client_token_set ? "badge--ok" : "badge--muted"}`}>
                {status.client_token_set ? "Client token configurado" : "Sin client token"}
              </span>
              ...
              <span className="badge badge--info">Entorno: {status.env}</span>
            </div>

            {!status.configured && (
              <div className="alert alert-warning" style={{ marginTop: "0.5rem" }}>
                Paddle no está configurado. Añade las variables de entorno al servidor:
                <pre style={{ marginTop: "0.5rem", fontSize: "0.82rem" }}>
{`PADDLE_ENV=sandbox
PADDLE_API_KEY=tu_api_key
PADDLE_CLIENT_TOKEN=tu_client_token
PADDLE_WEBHOOK_SECRET=tu_webhook_secret`}
                </pre>
              </div>
            )}
            ...
            <div style={{ marginTop: "0.4rem" }}>
              <p className="muted small" style={{ margin: 0 }}>
                <strong>URL de webhook a configurar en Paddle (Notifications):</strong>
              </p>
              <code style={{ fontSize: "0.85rem", wordBreak: "break-all" }}>
                {status.webhook_url}
              </code>
              <p className="muted small" style={{ marginTop: "0.4rem" }}>
                Eventos a activar: <code>subscription.created</code>,{" "}
                <code>subscription.updated</code>, <code>subscription.canceled</code>,{" "}
                <code>transaction.completed</code>, <code>transaction.payment_failed</code>,{" "}
            ...
            <div style={{ marginTop: "0.4rem" }}>
              <p className="muted small" style={{ margin: 0 }}>
                <strong>Precios:</strong> sincroniza producto y precios mensual/anual
                en cada tarifa desde{" "}
                <a href="/admin/tarifas">Tarifas → Editar → botón «Sync Paddle»</a>.
              </p>
            </div>
          </div>
        )}
      ...
      <section className="card settings-section">
        ...
        <p className="muted small" style={{ marginBottom: "0.75rem" }}>
          «Emitir abono» realiza el reembolso monetario en Paddle y genera la factura rectificativa en Alcurro automáticamente.
        </p>
        <DataTable
          data={tableData}
          columns={columns}
          loading={loading}
          exportFilename="cobros-paddle"
          height="500px"
